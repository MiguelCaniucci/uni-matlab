%TASK 3 
%Laguerre's method.
%polynomial coefficients

a = [-2, 6, 3, 7, 1];
x_ex = zeros(4,1);
sol = zeros(4,1);
rootsf = roots(a);
loop = 0;
f_test = @(x) a(1)*x^4 + a(2)*x^3 + a(3)*x^2 + a(4)*x + a(5);
j = 1;
order = 4;
x_arr = zeros(4,16);
y_arr = zeros(4,16);

while(loop < 4)

loop+1
ad = first_derivative(a);
add = second_derivative(a);

%function f(x)
f = @(x) a(1)*x^4 + a(2)*x^3 + a(3)*x^2 + a(4)*x + a(5);
%function f'(x)
fd = @(x) ad(1)*x^4 + ad(2)*x^3 + ad(3)*x^2 + ad(4)*x + ad(5);
%function f''(x)
fdd = @(x) add(1)*x^4 + add(2)*x^3 + add(3)*x^2 + add(4)*x + add(5);
%n - order of polynomial
n = 4-loop;

x = 3;
i = 1;
it = 100;

while(i < it)
    disp("whole...");
    %denominator with plus sign
    denp = (fd(x) - sqrt((n-1)*((n-1)*(fd(x)^2-n*f(x)*fdd(x)))));
    %denominator with minus sign
    denm = (fd(x) + sqrt((n-1)*((n-1)*(fd(x)^2-n*f(x)*fdd(x)))));
    
    if(abs(denp)<abs(denm))
        x_new = x - (n*f(x))/denm;
    else
        x_new = x - (n*f(x))/denp;
    end
    x_arr(loop+1, i) = x_new;
    y_arr(loop+1, i) = f_test(x_new);
    x = x_new;
    if(abs(f_test(x_new))<1e-6)
        i
        x_ex(j) = x_new;
        sol(j) = f_test(x_new);
        j = j + 1;
        i = it;

        %Horner division
        q = zeros(order+1, 1);
        for k = order : -1 : 1
            q(k) = a(order-k+1) + q(k+1)*x_new;
        end

        a(1:loop+1) = 0;
        for k = loop + 2 : order + 1 
            a(k) = q(order-k+2);
        end

    end
    i = i + 1;
end
loop = loop + 1;
end

plot(x_ex);
figure
plot(sol);
figure
fplot(f_test);
hold on
plot(x_ex(1), sol(1), 'r.');
plot(x_ex(2), sol(2), 'r.');
plot(x_ex(3), sol(3), 'r.');
plot(x_ex(4), sol(4), 'r.');
plot(1, 0, 'b*');

for i = 1 : 4
   disp("x" + i + " = ");
   for j = 1 : 16
       disp(x_arr(i,j));
   end 
   disp("y" + i + " = ");
   for j = 1 : 16
       disp(y_arr(i,j));
   end 
end

sol
i
x_ex
rootsf
